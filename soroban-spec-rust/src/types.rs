...
pub fn generate_union(spec: &ScSpecUdtUnionV0) -> TokenStream {
    ...
    if spec.lib.len() > 0 {
        ...
        let variants = spec.cases.iter().map(|c| {
            let name = match c {
                hcnet_xdr::ScSpecUdtUnionCaseV0::VoidV0(v) => v.name.clone(),
                hcnet_xdr::ScSpecUdtUnionCaseV0::TupleV0(t) => t.name.clone(),
            };
            let v_ident = format_ident!("{}", name.to_utf8_string_lossy());
            match c {
                hcnet_xdr::ScSpecUdtUnionCaseV0::VoidV0(_) => {
                    quote! { #v_ident }
                }
                hcnet_xdr::ScSpecUdtUnionCaseV0::TupleV0(t) => {
